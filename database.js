...
if (clientCount.count === 0) {
  ...
  const ic = db.prepare('INSERT INTO clients (name,company,status,revenue,spend,profit,expected_revenue,color) VALUES (?,?,?,?,?,?,?,?)');
  const c1 = ic.run('Priya Sharma','Growfast Retail','Active',316400,89000,227400,380000,'#FF6A00');
  const c2 = ic.run('Ravi Mehta','TechNova Labs','Active',245000,72000,173000,290000,'#3B82F6');
  const c3 = ic.run('Ananya Iyer','StyleHive','Active',198500,58000,140500,210000,'#10B981');
  const c4 = ic.run('Suresh Kumar','FoodBox India','Paused',145000,51000,94000,160000,'#8B5CF6');
  const c5 = ic.run('Neha Gupta','EduSpark','Active',82000,38000,44000,95000,'#F59E0B');
